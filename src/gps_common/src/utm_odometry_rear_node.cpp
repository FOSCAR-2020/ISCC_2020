...
void callback(const sensor_msgs::NavSatFixConstPtr& fix) {
  ...
  if (odom_pub) {
    ...
    tf.transform.translation.y = northing;
...
int main (int argc, char **argv) {
  ...
  odom_pub = node.advertise<nav_msgs::Odometry>("odom_rear", 10);

  ros::Subscriber fix_sub = node.subscribe("gps_rear/fix", 10, callback);
