...
class SlideWindow:
    ...
    def h_slidewindow(self, img, x_left_start, x_right_start):
        ...
        if steer_theta>28:
            ...
            steer_theta=-28
        center = (returns_x[size_center - 1], center_y[size_center - 1])
        length = np.sqrt(pow(abs(x_left - x_right), 2) + pow(abs(y_start), 2))
        return x_left_start, x_right_start, output_img ,steer_theta,center, length
